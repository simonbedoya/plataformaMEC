...
module.exports = {
    ...
    query_insertFile: "INSERT INTO TBL_FILE (PK_SENSOR, PATH_FILE, DATE_FILE, HOUR_FILE, REGISTERDATE_FILE) VALUES (${pk_sensor},'${path_file}', '${date}', '${hour}', '${reg_date}')",
    query_verifyExistFile: "SELECT count(tbf.PK_FILE) AS counter, (SELECT tbl.UPLOADPATH_LOCATION FROM TBL_LOCATION tbl WHERE tbl.PK_SENSOR = ${pk_sensor} AND tbl.STATUS_LOCATION = 'Activa') as UPLOADPATH FROM TBL_FILE tbf WHERE tbf.PK_SENSOR = ${pk_sensor} AND tbf.DATE_FILE = '${date}' AND tbf.HOUR_FILE = '${hour}'",
    query_updateFile: "UPDATE TBL_FILE SET PATH_FILE = '${path_file}', REGISTERDATE_FILE = '${reg_date}' WHERE PK_SENSOR = ${pk_sensor} AND DATE_FILE = '${date}' AND HOUR_FILE = '${hour}'",
