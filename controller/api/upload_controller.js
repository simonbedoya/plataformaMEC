    ...
    insertFile: function (pk_sensor, path_file, file_name, pk_location) {
        return new Promise(
            function (fullfill) {
                ...
                db.query(template(sqlQuery.query_insertFile,{pk_sensor: pk_sensor, path_file: path_file, date: date, hour: hour, reg_date: reg_date, pk_location: pk_location}), function (err, result) {
    ...
    updateFile: function (pk_sensor, path_file, file_name) {
      return new Promise(
          function (fullfill) {
              ...
              let hour = functions.convertHour(file_name_array[1]);
              let reg_date = functions.datetime();
              db.query(template(sqlQuery.query_updateFile,{pk_sensor: pk_sensor, path_file: path_file, date: date, hour: hour, reg_date: reg_date}), function (err, result) {
    ...
    verifyExistFile: function (file_name,pk_sensor) {
        return new Promise(
            function (fullfill) {
                ...
                let date = functions.convertDate(file_name_array[0]);
                let hour = functions.convertHour(file_name_array[1]);
                db.query(template(sqlQuery.query_verifyExistFile,{pk_sensor: pk_sensor, date: date, hour: hour}), function (err, result) {
